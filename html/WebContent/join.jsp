...
function Timer(){     //시간표및 조건검사
	...
	$("#m_timer").html("남은시간 : " + temp);
	if(tcounter<0) tstop();    // 3분후 완료
}
 
 function tstop(){ //완료함수
	clearInterval(t1);
	$("#m_timer").html("시간이 초과되었습니다.");
...
function start_phone_auth()
{
	...
	if(formnm.auth_code.value == "보안문자 입력해 주세요"){alert("보안문자를 입력해 주세요.");return;}
	if(!isValidType(formnm.auth_code, 'text', '보안문자를 입력해 주세요.' )){ return; }
...
function check_phone_auth()
{
	...
	if(!isValidType(formnm.u_mtel_code, 'text', '인증번호를 입력하여 주세요.' )){ return; }
...
{
	tstop();
	$("#mtel_auth_area").html("● 휴대폰 인증이 완료되었습니다.");
}
...
<!-- 2016-05-06  임시팝업 - end-->
		...
		<div style="margin:0 auto; width:554px; overflow:hidden;">
			<span class="top-web">
				<!--
				듀오는 <span class="logo duo"></span>
				웨드는 <span class="logo wed"></span>
				리메리는 <span class="logo remarry"></span>
				텐웨딩홀은 <span class="logo ten"></span>
				-->
			...
			<div class="box-wrapper">
				...
				<table cellspacing="0" cellpadding="0" summary="회원가입 폼입니다." style="width:100%;">
					...
					<tbody>
						<tr>
							<td>
								<input type="text" placeholder="아이디" title="아이디" value="아이디" onfocus="if(this.value==this.defaultValue)this.value='';" onblur="if(this.value=='')this.value=this.defaultValue;" class="ip_id" id="u_id" name="u_id" maxlength="10" onKeyup="checkRegisted()" />
						...
						<tr>
							<td>
								<input type="text" placeholder="비밀번호 영문/숫자/특수문자 조합으로 8자 이상 15자 이하" title="비밀번호" value="비밀번호 영문/숫자/특수문자 조합으로 8자 이상 15자 이하" class="ip_pw" name="u_pwd" id="u_pwd" maxlength="15" />
								<!-- 해당 부분은 현재 듀오 회원가입 쪽 노출 부분 처럼 진행한다고 합니다. -->
								<span id="pwdCheck" style="display:none;" class="pwcheck">영문/숫자/특수문자 조합으로 8자 이상 15자 이하 , 사용가능 특수문자 : ~!@#$%^&amp;*()-+=</span>
							</td>
						</tr>
						<tr>
							<td>
						...
						<tr>
							<td>	
								...
								<div style="float:right; width:170px;">
									...
									<label class="ip_sex radio_sex"><input type="radio" title="남자" name="u_sex" value="0" />남자</label>
									<label class="ip_sex radio_sex"><input type="radio" title="여자" name="u_sex" value="1" />여자</label>
								</div>
							</td>
						</tr>
						<tr id="mtel_auth_area">
							<td id="mtel_state" style="padding:0;">
								...
								<div id="image_code" style="width:100%;height:36px;padding:10px 0;">
									...
									<img  src="/html/membership/new_img/ic_num_01.jpg" alt="보안문자" class="auth_img" style="display:block;float:left;margin:0 10px;height:35px;border:1px solid #d9d9d9;"/>	
									<a onclick="change_auth_code()" style="display:block;float:left;"><img src="/html/membership/new_img/btt_num_01.jpg" alt="새로고침" style="display:block;height:35px;background:#f5f6f7;" 
									/></a>
									<a onclick="start_phone_auth()" style="cursor:pointer;" class="confirm" id="bt_mtel_auth" >인증받기</a>
								</div>

								<div id="sms_code_input" style="width:100%;height:70px;display:none;">
									<li style="width:100%;height:50px;">
									<input type="text" placeholder="인증번호" title="인증번호" value="인증번호" onfocus="if(this.value==this.defaultValue)this.value='';" onblur="if(this.value=='')this.value=this.defaultValue;" class="ip_hp" name="u_mtel_code" style="width:150px;" /><a onclick="check_phone_auth()" style="cursor:pointer;" class="confirm" id="check_bt1">확인</a>
									</li>
						...
						<tr>
							<td>
								...
								<select class="ip_duo" title="결혼경력" name="u_married">
									...
									<option value="5">미혼</option>
                                    <option value="2">재혼</option>
								</select>
...
<option value="34">기타</option>
								...
								<select class="ip_area" title="지역선택" name="u_dwell">
